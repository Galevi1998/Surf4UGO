...
router.post('/', async (req, res) => {
  try {
    ...
    if (sortBy === 'product-name') {
      products = await Product.find({quantity: { $gt: 0 } }).sort({ name: 1 });
    } else if (sortBy === 'product-price') {
      products = await Product.find({quantity: { $gt: 0 } }).sort({ price: 1 });
    } else if (sortBy === 'product-brand') {
      products = await Product.find({quantity: { $gt: 0 } }).sort({ brand: 1 });
    } else {
