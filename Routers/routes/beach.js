...
router.get('/', async (req, res) => {
  try {
    const products = await Product.find({ category: 'beach products', quantity: { $gt: 0 } });
    res.render('beach', { products });
  } catch (error) {
    console.error(error);
...
router.post('/', async (req, res) => {
  try {
    ...
    if (sortBy === 'product-name') {
      products = await Product.find({ category: 'beach products', quantity: { $gt: 0 } }).sort({ name: 1 });
    } else if (sortBy === 'product-price') {
      products = await Product.find({ category: 'beach products', quantity: { $gt: 0 } }).sort({ price: 1 });
    } else if (sortBy === 'product-brand') {
      products = await Product.find({ category: 'beach products', quantity: { $gt: 0 } }).sort({ brand: 1 });
    } else {
